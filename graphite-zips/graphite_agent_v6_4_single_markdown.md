...
class Git:
        ...
        return (
            subprocess.run(
                [
                    ...
                    "merge-base",
                    "--is-ancestor",
                    str(self.resolve(ancestor)),
                    str(self.resolve(child)),
                ],
    ...
    def merge_base(self, left, right):
        out = self.run(
            ["git", "merge-base", str(self.resolve(left)), str(self.resolve(right))],
            check=False,
        )
        return out or None
    ...
    def commit_distance(self, start, end):
        out = self.run(
            [
                ...
                "rev-list",
                "--count",
                f"{self.resolve(start)}..{self.resolve(end)}",
            ],
            check=False,
        )
...
class RelationshipCollector:
    ...
    def owner(self, branch):
        owners = [
            r for r in self.cfg.configured_roots if self.git.is_ancestor(r, branch)
        ]
        return owners[0] if len(owners) == 1 else None

    def edge(
        self, a, b, edge_type, classification, status, confidence, evidence, root=None
    ):
        return Edge(
            self.ids.rel(),
            a,
                ...
                for line in self.git.merge_commits_between(root, branch):
                    ...
                    for parent_sha in parents[1:]:
                        known = next(
                            (
                                name
                                for name, pr in branch_map.items()
                                if pr.head_ref_oid == parent_sha
                            ),
                            None,
                        )
                        if known:
...
class TopologyAuditEngine:
    ...
    def root_ownership(self, branch):
        ...
        for root in self.cfg.configured_roots:
            ...
            if distance is not None:
                candidates.append(
                    {"root": root, "distance": distance, "merge_base": merge_base}
                )
        candidates.sort(key=lambda x: x["distance"])
        ambiguous = (
            len(candidates) > 1
            and candidates[0]["distance"] == candidates[1]["distance"]
        )
    ...
    def sibling_patch_overlap(self, branch, root, byroot):
        ...
        return any(
            sibling != branch and bool(mine & self.git.patch_ids_between(root, sibling))
            for sibling in byroot.get(root, [])
        )
    ...
    def audit(
        ...
        invariants = {
            ...
            "parent_exists": bool(
                parent and (parent in branch_map or parent in self.cfg.configured_roots)
            ),
        ...
        return Audit(
            ...
            failed,
            cycle_path,
            self.merge_analysis(branch, edges),
        )
...
class Classifier:
    ...
    def classify(self, pr, edges, audit, topo):
        ...
        root = (
            executable[0].root_branch if executable else audit.root_owner.get("owner")
        )
    ...
    def run(self):
        ...
        for p in prs:
            ...
            if branch in self.cfg.configured_roots:
                continue
            inbound = [
                e for e in edges if e.to_ref == branch and self.policy.executable(e)
            ]
            parent = inbound[0].from_ref if inbound else None
            topo = topology.get(branch, {"sources": [], "targets": []})
            ghost = audit_engine.sibling_patch_overlap(
                branch, roots[branch]["owner"], byroot
            )
...
class GraphiteClient:
    ...
    def verify_parent(self, branch, parent):
        ...
        if actual != parent:
            raise RuntimeError(
                f"Graphite parent mismatch for {branch}: expected {parent}, got {actual}"
            )
        return True
