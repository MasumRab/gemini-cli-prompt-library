    ...
    for n in nodes.values():
        ...
        if r and r not in cand:
            cand[r] = {
                ...
                "score": 70,
                "confidence": "medium",
                "signals": ["root_branch in analysis"],
            }
...
def target_analyse(nodes, candidate_targets):
    ...
    for b, n in nodes.items():
        ...
        cat = (
            "target_confirmed"
            if declared == inferred or not declared
            else (
                "wrong_pr_target_candidate"
                if inferred and declared != inferred
                else "target_intent_required"
            )
        )
        ...
        if req:
            ...
            opts = []
            if declared:
                opts.append("target=" + declared)
            if inferred:
                opts.append("target=" + inferred)
            if declared and inferred and declared != inferred:
                opts.append(f"targets={declared}+{inferred}")
            ...
            qs.append(
                {
                    ...
                    "options": opts,
                    "recommended_option": (
                        "target=" + inferred if inferred else "leave_triage"
                    ),
                    "confidence": "medium",
                }
            )
            ...
            "candidate_targets": [
                {
                    "target": x,
                    "confidence": "medium",
                    "evidence": ["declared/inferred target"],
                }
                for x in [declared, inferred]
                if x
            ],
            "confirmed_targets": [],
        ...
        rec[b] = {
            "recommended_action": (
                "resolve_target_intent" if req else "keep_declared_target"
            ),
            "confidence": "medium",
            "because": [cat],
        }
