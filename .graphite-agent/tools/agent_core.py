...
from lib.targets import (
    discover_targets as lib_discover_targets,
    target_analyse as lib_target_analyse,
)
...
def snap():
    s = rj(OUTPUTS_DIR / "analysis_snapshot.json") or rj(
...
def triage_packets(s, rel):
    ...
    for b, n in nodes(s).items():
        ...
        cat = n.get("diagnostic_category") or (
            "in_target_conflict_resolution_merge"
            if n.get("status") == "blocked_merge_commits" and "merge" in reason.lower()
            else n.get("status")
        )
        out[b] = {
            "id": f"triage-{i:06d}",
            ...
            "relationship_edges": [
                e["id"] for e in rel["edges"] if e.get("from") == b or e.get("to") == b
            ],
            "recommended_action": (
                "linearise_before_graphite_tracking"
                if cat == "in_target_conflict_resolution_merge"
                else "manual_review_required"
            ),
            "next_steps": [
                f"explain.py --branch {b}",
                f"questions.py --branch {b}",
                "validate_plan.py",
            ],
        }
    return out
...
def analyse_outputs():
    s = snap()
    p = plan()
    rel = relationship_graph(s)
    tri = triage_packets(s, rel)
    qs = questions(tri)
    rec = {
        b: {
            "branch": b,
            "recommended_action": (
                (
                    "track_and_restack"
                    if n.get("status") == "needs_restack"
                    else "track_only"
                )
                if n.get("status") in EXECUTABLE
                else "block_or_ask_user"
            ),
            "because": [n.get("reason")],
        }
        for b, n in nodes(s).items()
    }
    for name, obj in [
        ("analysis_summary.json", summary(s, p)),
        ("relationship_graph.json", rel),
        ("triage_packets.json", tri),
        ("question_queue.json", qs),
        ("recommendations.json", rec),
    ]:
...
def discover_targets():
    s = snap()
    configured_roots = s.get("metadata", {}).get("configured_roots", [])
...
def target_analyse():
    ...
    branches, qs, rec = lib_target_analyse(nodes(s), targets["candidates"])
    wj(
        OUTPUTS_DIR / "target_matrix.json",
        {"generated_at_utc": now(), "branches": branches},
    )
...
def root_health():
    s = snap()
    targets = discover_targets()["candidates"]
...
def stack_order():
    s = snap()
    rh = rj(OUTPUTS_DIR / "root_health.json") or {"roots": root_health()}
...
def validate_plan():
    analyse_outputs()
    tri = rj(OUTPUTS_DIR / "triage_packets.json", {})
    p = plan()
    fail = []
    for e in p.get("execution_queue", []):
        if e.get("status") not in EXECUTABLE:
            fail.append({"id": "unsafe-status", "branch": e.get("branch")})
        if not e.get("resolved_parent"):
            fail.append({"id": "missing-parent", "branch": e.get("branch")})
        if e.get("branch") in tri:
            fail.append(
                {"id": "branch-in-triage-and-execution", "branch": e.get("branch")}
            )
