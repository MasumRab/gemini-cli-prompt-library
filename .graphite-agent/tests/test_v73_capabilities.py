...
class V73Tests(unittest.TestCase):
    ...
    def test_target_discovery(self):
        self.assertIn("scientific", self.core.discover_targets()["candidates"])

    def test_target_matrix(self):
        self.assertIn("feature/science-a", self.core.target_analyse())

    def test_root_health(self):
        self.assertEqual(self.core.root_health()["scientific"]["health"], "stale")

    def test_stack_blocks_stale_root(self):
        self.core.root_health()
        self.assertFalse(
            self.core.stack_order()["targets"]["scientific"]["execution_allowed"]
        )
