...
if os.path.exists(p4_path) and os.path.exists(p5_path):
    ...
    root_blocks = re.findall(
        r"(## .*?(?:root_health|root_refresh|root_questions|root_decide).*?)(?=\n## |\Z)",
        p4_content,
        re.DOTALL,
    )
    new_p4 = p4_content
...
if os.path.exists(p3_path):
    ...
    for tool in v72_tools:
        new_p3 = re.sub(
            r"\.graphite-agent/tools/" + re.escape(tool) + r"\n?", "", new_p3
        )
...
"""
    ...
    if "8.6 Universal Decision Interface" not in p8_content:
        with open(p8_path, "a") as f:
            f.write(decision_glue)
